...
public class DatabaseHelper {
    ...
    private void initializeTables() throws SQLException {
        ...
        // === NEW: Table for storing question reviews (TP3 feature) ===
    ...
    // Q&A section methods and allignment with requirements
    ...
    // outputs all users from the DB that are registered to give user the option to send the msg to a specific person
    public List<String> getAllUsernames() {
        List<String> users = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement("SELECT USERNAME FROM USERS ORDER BY USERNAME ASC")) {
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    // These are the new updates made as we are on tp3

    public boolean addReview(int qid, String reviewer, String text, String reliability) {
        try (PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO REVIEWS (QID, REVIEWER, TEXT, RELIABILITY) VALUES (?, ?, ?, ?)")) {
            ps.setInt(1, qid);
            ps.setString(2, reviewer);
            ps.setString(3, text);
            ps.setString(4, reliability);
            ps.executeUpdate();
            saveChanges();
            return true;
        } catch (SQLException e) {
    ...
    public List<String[]> getReviewsForQuestion(int qid) {
        List<String[]> list = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement(
                "SELECT REVIEWER, TEXT, RELIABILITY, CREATED_AT FROM REVIEWS WHERE QID = ?")) {
            ps.setInt(1, qid);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new String[]{
    ...
    public List<String[]> getReviewsByReviewer(String reviewer) {
        List<String[]> list = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement(
                "SELECT QID, TEXT, RELIABILITY, CREATED_AT FROM REVIEWS WHERE REVIEWER = ?")) {
    ...
    // returns questions reviewed if reviewed by reviewers for all users
    // addresses the reviewed table and only filters out those submissions for search
    public List<String[]> getQuestionsReviewedByReviewers() {
        List<String[]> list = new ArrayList<>();
        try (PreparedStatement ps = conn.prepareStatement(
                "SELECT DISTINCT q.QID, q.USERNAME, q.TITLE, q.BODY, q.CREATED_AT " +
                "FROM QUESTIONS q " +
                "JOIN REVIEWS r ON q.QID = r.QID " +
                "JOIN ROLES ro ON r.REVIEWER = ro.USERNAME " +
                "WHERE LOWER(ro.ROLE) = 'reviewer' " +
                "ORDER BY q.QID DESC")) {
    ...
    // Allows me to mark a question deemed as reviewed if an entry requires a review..
    public boolean markQuestionReviewed(int qid) {
        try (PreparedStatement ps = conn.prepareStatement(
                "UPDATE QUESTIONS SET BODY = BODY || '\n\n[Reviewed ]' WHERE QID = ?")) {
            ps.setInt(1, qid);
